...
export default function CameraView({
  ...
  // Restart render loop when renderFrame callback changes (shade/opacity/status change)
  useEffect(() => {
    cancelAnimationFrame(rafRef.current);
    rafRef.current = requestAnimationFrame(renderFrame);
  ...
  return (
    <div className="camera-view-container">
      ...
      {(status === "active" || status === "photo") && (
        <div
          className={`face-oval-frame ${faceDetected ? "face-oval-locked" : "face-oval-seeking"}`}
          aria-hidden
        >
          <svg
            viewBox="0 0 200 260"
            xmlns="http://www.w3.org/2000/svg"
            className="face-oval-svg"
          >
            <defs>
              <filter id="oval-glow">
            ...
            {/* Outer dimming mask cutout */}
            <ellipse
              cx="100" cy="128" rx="76" ry="100"
              fill="none"
              stroke={faceDetected ? "rgba(134,239,172,0.85)" : "rgba(255,182,193,0.7)"}
              strokeWidth="2.5"
              strokeDasharray={faceDetected ? "none" : "8 5"}
              filter="url(#oval-glow)"
              className="oval-ellipse"
            />
            {/* Corner tick marks */}
            {[
              [24, 28], [176, 28], [24, 228], [176, 228]
            ].map(([cx2, cy2], i) => (
              <circle key={i} cx={cx2} cy={cy2} r="3"
                fill={faceDetected ? "rgba(134,239,172,0.9)" : "rgba(255,182,193,0.8)"}
              />
            ))}
          </svg>
          ...
          {/* ✅ Face locked badge */}
          {faceDetected && (
            <div className="face-locked-badge">
              <span className="face-locked-dot" />
              Face detected
            </div>
          )}
        </div>
      ...
      {/* ── SELECTED SHADE NAME OVERLAY (top centre of camera) ── */}
      {selectedShade && (status === "active" || status === "photo") && (
        <div className="shade-name-chip" aria-live="polite">
          <span
            className="shade-name-chip-dot"
            style={{ backgroundColor: selectedShade.hexColor }}
          />
          <span className="shade-name-chip-text">{selectedShade.name}</span>
          <span className="shade-name-chip-finish">{selectedShade.finish}</span>
        </div>
      )}
